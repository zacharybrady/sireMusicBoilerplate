...
<?php } else{
	...
	if(is_page_template('page_templates/home.php') ) { //Home page
	...
	else if(is_single() && get_post_format() == "") { //Single post
		require_once(  'css/critical/critical-post.css');
	}
	else if(is_single() && get_post_format() == "audio") { //Single audio post
		require_once(  'css/critical/critical-postAudio.css');
	}
	else if(is_single() && get_post_format() == "video") { //Single video post
		require_once(  'css/critical/critical-postVideo.css');
	}
	else if(is_single() && get_post_format() == "gallery") { //Single video post
		require_once(  'css/critical/critical-postGallery.css');
	}
	else if(is_page_template('page_templates/music.php')) { //Single video post
		require_once(  'css/critical/critical-music.css');
	}
	else if(is_page_template('page_templates/store.php')) { //Single video post
		require_once(  'css/critical/critical-store.css');
	}
	else if(is_page_template('page_templates/videos.php')) { //Single video post
		require_once(  'css/critical/critical-video.css');
	}
	else {
...
function my_embed_oembed_html($html, $url, $attr, $post_id) {
	...
	return '<div class="h-ombed">' . $html . '</div>';
}
...
/**
 * Changes the markup for Wordpress galleries. 
 *
 *
 * @since Siren 3.6
 *
...
function my_post_gallery( $output, $attr) {
    ...
    $output .= "<div class='gallery_row lazyload' data-script='" . get_bloginfo('template_directory') ."/js/gallery.min.js' >";

    $i = 0;
    foreach ( $attachments as $id => $attachment ) {
        $link = isset($attr['link']) && 'file' == $attr['link'] ? wp_get_attachment_link($id, $size, false, false) : wp_get_attachment_link($id, $size, true, false);

        $link = str_replace("<a", '<a data-featherlight="'.wp_get_attachment_url().'" ', $link);
